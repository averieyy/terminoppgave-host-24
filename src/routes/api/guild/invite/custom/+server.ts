...
export const POST : RequestHandler = async ({ cookies, request }) => {
  ...
  const existinginvite = await DatabaseConnection.queryOne<{ uuid: string }>('SELECT * FROM invitation WHERE uuid = $1::text OR custom = $1::text', custom);
  if (existinginvite) return json({ message: 'An invitation with that link already exists' }, { status: 409 });

  const uuid = randomUUID();

  const success = await DatabaseConnection.queryOne<{ uuid: string }>('INSERT INTO invitation (guildid, uuid, custom) VALUES ($1::integer, $2::text, $3::text) RETURNING uuid;', guildid, uuid, custom);
