...
export const POST: RequestHandler = async ({ cookies, request }) => {
  ...
  if (!target.administrator) return json({ message: 'User isn\'t an administrator' }, { status: 409 });
  
  await DatabaseConnection.execute('UPDATE guildmembers SET administrator = false WHERE guildid = $1::integer AND userid = $2::integer', guildid, target.userid);

  return json({ message: 'Successfully demoted member' }, { status: 200 });
}